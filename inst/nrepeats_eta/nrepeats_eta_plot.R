...
full_couple_prob_trajectories_crn <-
  foreach(t_dist_df = seq(2,2,0.2), .combine = rbind) %:% 
  foreach(nrepeats_eta = seq(1,10,2), .combine = rbind) %:% 
  foreach(i = 1:iterations, .combine = rbind) %dopar% {
    ...
    return(data.frame(n, p, iteration = i, t_dist_df=t_dist_df, nrepeats_eta=nrepeats_eta,
                      t= c(1:length(tvUB_full)), metric_d=tvUB_full))
  }

# save(full_couple_prob_trajectories_crn, file = "inst/nrepeats_eta/nrepeats_eta_plot_crn.RData")
# load("inst/nrepeats_eta/nrepeats_eta_plot_crn.RData")

plots_nrepeats_eta <- full_couple_prob_trajectories_crn %>%
  dplyr::group_by(n, p, t_dist_df, nrepeats_eta, t) %>%
  dplyr::summarise(metric_d=mean(metric_d), count=n())
nrepeats_eta_plot_crn <-
  ggplot(plots_nrepeats_eta, 
         aes(y=metric_d, x=t, color=as.factor(nrepeats_eta)),
  ...
  scale_y_log10() + 
  scale_colour_viridis_d(name=TeX('Slice sampling repeats for $\\eta$')) +
